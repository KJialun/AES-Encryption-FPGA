...
entity XOR_calc is
    Port ( 
        ...
        xor_out       : out STD_LOGIC_VECTOR(7 downto 0):= (others => '0') --input 8 bits         
    );
end XOR_calc;
...
signal  xor_calculate  :  STD_LOGIC_VECTOR(7 downto 0):= (others => '0');    --internal 8 bits   
...
byte_out3 <= byte_in3;
byte_out4 <= byte_in4;
xor_out <= xor_calculate;


 Xor_calc : process(clk, rst,en)
    begin
        if (rst = '1') then
        xor_calculate <= (others => '0'); --all output byte become 0
        elsif (rising_edge(clk)) and (en = '1')  then   --shift rowns
--            if (en = '1') or (rising_edge(en)) then
                xor_calculate <= byte_out1 xor byte_out2 xor byte_out3 xor byte_out4; 
--            end if;      
        end if;
    end process; 
